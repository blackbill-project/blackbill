...
#if defined(NDEBUG)
#error "Blackbill cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ublackbill) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >= 40 && nHeight < 30040) {
        nSubsidy = 10 * COIN; // Normal block
    }
    else if (nHeight >= 30040 && nHeight < 35040) {
        nSubsidy = 25 * COIN; // Normal block
    }
    else if (nHeight >= 35040 && nHeight < 808400) {
        nSubsidy = 7.2 * COIN; // Normal block
    }
    else if (nHeight >= 808400 && nHeight < 808410) {
        nSubsidy = 95000000 * COIN; // Increase supply to 1 Billion
    }
    else if (nHeight >= 808410 && nHeight < 1364901) {
        nSubsidy = 7.2 * COIN; // Normal block
    }
    else {
      nSubsidy = 0.05 * COIN; // Normal block
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    /*if (nHeight < Params().LAST_POW_BLOCK())
		...
		ret = blockValue * 0.7 * COIN; // Normal block
    }*/
    ...
    if (nHeight < 40) {
        ...
        ret = 0; // Increase supply to 1 Billion
    }
    else if (nHeight >= 808410 && nHeight < 1364901) {
        ret = 5.04 * COIN; // Normal block
    }
...
{
    RenameThread("blackbill-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BIL: We use certain sporks during IBD, so check to see if they are
